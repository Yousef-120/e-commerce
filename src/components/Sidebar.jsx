  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed left-0 top-0 h-full w-[80%] bg-white px-5 py-8 flex flex-col gap-4 z-50 border-r border-black/10"
      >
        {/* Top */}
        <div className="top flex justify-between gap-2">
          ...
          {/* Search */}
          <input className="w-full outline-0 py-3 px-4 rounded-full bg-[#F0F0F0]" type="search" name="" id="" placeholder="Search for products..." />
        </div>

        {/* Links */}
        <div className="flex flex-col gap-4 text-lg font-medium text-black mt-2">
          {navLinks.map((link) => (
            <Link to={link.linkUrl} className="hover:opacity-60 transition">
              {link.linkName}
            </Link>
          ))}
        </div>
